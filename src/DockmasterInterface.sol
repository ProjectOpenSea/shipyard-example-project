...
interface DockmasterInterface {
    ...
    event TraitUpdated(
        bytes32 indexed traitKey, uint256 indexed tokenId, bytes32 trait
    );
    event TraitUpdatedBulkConsecutive(
        bytes32 indexed traitKeyPattern, uint256 fromTokenId, uint256 toTokenId
    );
    event TraitUpdatedBulkList(
        bytes32 indexed traitKeyPattern, uint256[] tokenIds
    );
    event Transfer(
    ...
    function getTraitKeyAt(uint256 index)
        external
        view
        returns (bytes32 traitKey);
    function getTraitKeys()
        external
        view
        returns (bytes32[] memory traitKeys);
