...
contract Dockmaster is AbstractNFT {
        ...
        AbstractNFT(__name, __symbol)
    {
        ...
        emit Hail(
            string(
                abi.encodePacked(
                    "Hi Mom! I'm deploying my very own",
                    __name,
                    " NFT contract!"
                )
            )
    ...
    function stringURI(uint256 tokenId) internal view returns (string memory) {
        return json.objectOf(
            Solarray.strings(
                json.property(
                    "name",
                    string.concat("Dockmaster NFT #", tokenId.toString())
                ),
                json.property(
                    "description",
                    string.concat(
        ...
        returns (string memory)
    {
        ...
        string[] memory combined =
            new string[](staticTraits.length + dynamicTraits.length);
     ...
     */
    function _staticAttributes(uint256 tokenId)
        internal
        view
        virtual
        override
    ...
    function _image(uint256 tokenId)
        internal
        pure
        override
        returns (string memory)
    {
        return svg.top({
            props: string.concat(
                svg.prop("width", "500"), svg.prop("height", "500")
                ),
            children: string.concat(
                ...
                svg.text({
                    ...
                    children: string.concat(
                        "You're looking at slip #", tokenId.toString()
                        )
                })
                )
        });
    ...
    function mint(address to) public {
        // Only addresses with the first byte of their address equal to 0 can
        // mint.
        if ((uint256(uint160(msg.sender)) >> (160 - 8)) > 0) {
    ...
    function _isOwnerOrApproved(uint256 tokenId, address addr)
        internal
        view
        virtual
        override
        returns (bool)
    {
        return ownerOf(tokenId) == addr || getApproved(tokenId) == addr
            || isApprovedForAll(ownerOf(tokenId), addr);
    }
